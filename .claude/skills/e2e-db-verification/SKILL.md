...
7. **If the change touches ordering/serialization of concurrent writes (e.g. anything near `saveQueues` in `server/index.js`), test the race directly — don't just test the happy sequential path.** A sequential test (save, wait, save again) can pass even when a fix does nothing, because it never creates the interleaving the fix is supposed to handle. Instead: fire two (or more) real client saves back-to-back with *no* await/delay between them (e.g. two rapid `window.CaptureGhost()`-triggered saves, or two direct `socket.emit('save_game_state', ...)` calls issued in the same tick with deliberately different payloads so you can tell which one "won"), then query the database directly and confirm the **last-emitted** payload is what persisted — not just that *a* save persisted. This project's real `saveQueues` bug (found 22/08/2026) passed every sequential test that existed at the time; it only reproduced when two emits landed close enough together that query completion order could differ from emission order. If you can't make the race happen on demand by racing real emits, you haven't verified the fix — you've verified the easy case. For the causal analysis behind *why* a given serialization fix is scoped correctly (right key, right granularity), see the `forensic-root-cause-analysis` skill.

8. **Clean up.** `DELETE FROM players WHERE email = $1` (cascades to `characters` via the FK). Verify the count is back to zero — don't just fire the delete and assume it worked.
