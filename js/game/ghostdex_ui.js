...
function RenderGhostdexInNavbar(db) {
    ...
    html += '<button onclick="DebugUnlockEpisode1()" style="margin-top:5px; padding:4px 10px; background:#FF0000; color:#FFF; font-weight:bold; border:1px solid #FFF; border-radius:3px; font-size:10px; cursor:pointer;">DEV DEBUG: UNLOCK EP.1 GHOSTS</button>';
    html += '</div>';
    ...
    db.forEach(function(ghost) {
        ...
        if (st === 0) {
            ...
            bg = '#1a1a2e'; border = '#9932CC'; opacity = '0.8';
        } else {
        ...
        if (st === 0) {
            ...
            html += '<div style="margin:4px 0;"><img src="assets/sprites/ghost_' + ghost.id + '_r.webp" style="width:24px; height:24px; image-rendering:pixelated; filter:brightness(0);" /></div>';
            html += '<div style="color:#9932CC; font-size:10px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">' + ghost.nome + '</div>';
        } else {
...
window.ShowGhostdexDetail = function(ghostId) {
    ...
    if (st === 0) return; // Can't view undiscovered
...
window.DebugUnlockEpisode1 = function() {
    var ids = [];
    for (var i = 1; i <= 101; i++) {
        ids.push(i.toString().padStart(3, '0'));
    }
    ids.forEach(function(id) {
        window.UpdateGhostdex(id, 2);
    });
    alert("Cheat ativado: Todos os 101 fantasmas desbloqueados!");
